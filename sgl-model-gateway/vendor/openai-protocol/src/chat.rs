...
fn validate_messages(messages: &[ChatMessage]) -> Result<(), validator::ValidationError> {
    ...
    // [vessl] Empty user content is legal: OpenAI's API accepts content: ""
    // (and an empty parts array), and the engines this gateway fronts accept
    // and answer both. The published crate rejects them, which made the
    // gateway stricter than the protocol it implements — clients got a 400
    // for a request OpenAI serves (contract defect D-3).
    Ok(())
}
