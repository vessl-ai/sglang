...
class TestCalculateResponseUsage(CustomTestCase):
    def test_cache_report_on_zero_cached(self):
        ...
        self.assertEqual(
            usage.prompt_tokens_details, PromptTokensDetails(cached_tokens=0)
        )
        self.assertEqual(usage.prompt_tokens_details.model_dump(), {"cached_tokens": 0})
        self.assertEqual(
            usage.model_dump()["prompt_tokens_details"], {"cached_tokens": 0}
        )

    def test_cache_report_on_nonzero_cached(self):
        responses = [
            {
                "meta_info": {
                    "prompt_tokens": 10,
        ...
        self.assertEqual(usage.prompt_tokens_details.model_dump(), {"cached_tokens": 5})
        ...
        responses = [
            ...
            },
            {
                "meta_info": {
                    "prompt_tokens": 10,
                    "completion_tokens": 1,
                    "cached_tokens": 999,  # second choice of prompt 0, must be ignored
                }
            },
            {
                "meta_info": {
            ...
            },
            {
                "meta_info": {
                    "prompt_tokens": 10,
                    "completion_tokens": 1,
                    "cached_tokens": 999,  # second choice of prompt 1, must be ignored
                }
            },
        ]
        usage = UsageProcessor.calculate_response_usage(
...
class TestCalculateStreamingUsage(CustomTestCase):
    ...
    def test_cache_report_on_nonzero_cached(self):
        ...
        self.assertEqual(
            usage.prompt_tokens_details.model_dump(), {"cached_tokens": 10}
        )
...
class TestCalculateTokenUsage(CustomTestCase):
    def test_cached_zero_details_plus_multimodal_still_attach(self):
        ...
        cached_details = UsageProcessor._cached_details(0)
