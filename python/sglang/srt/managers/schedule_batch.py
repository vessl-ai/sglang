...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def _mamba_radix_cache_v2_req_prepare_for_extend(
        ...
        if mask:
            ...
            # Record the slot before the swap: the cache-insertion path wants the
            # last COMMITTED state (consumer), not the next write target
            # (producer), and under overlap those diverge. The lazy branch below
            # leaves mamba_next_track_idx alone, which is why this is recorded
            # ahead of the branch rather than inside it.
            req.mamba_last_track_idx = req.mamba_next_track_idx
            # Upstream reads the lazy flag through a module-level helper; this
            # branch still has server_args in scope and no such helper.
