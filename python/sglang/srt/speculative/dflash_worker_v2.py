    ...
    def _accept_block(
        ...
        *,
        ...
        if self._selector_sample is not None:
            # Same TP-divergence hazard as the branches below: sample_path()
            # (selector.py) draws its uniforms from each rank's own RNG state,
            # so candidate_ids/q_rows -- and the accept_len/bonus derived from
            # them -- are not guaranteed identical across ranks. Broadcast from
            # rank 0 before committing, same as every other branch here.
            # fork PR#36 TP-sync semantics extended to selector-sampling accept;
            # runtime-validated only for non-selector paths -- revalidate when
            # selector sampling is enabled.
