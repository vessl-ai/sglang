...
def _resolve_dflash_aux_hidden_state(
    *,
    ...
    if spec_algorithm.is_dflash_family() and not is_draft_worker:
        ...
        # Mirrors the gate inside _map_muse_target_layer_ids: when it fires, the
        # ids above are HF layer-output ids rather than target layer indices, so
        # the range/attention-kind validation below cannot be read into them.
        ids_are_shifted = getattr(
            model_config.hf_config, "model_type", None
        ) == "muse_glimmer" and bool(
            _MUSE_LAYER_OUTPUT_DRAFT_ARCHITECTURES.intersection(
